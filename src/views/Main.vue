...
<template>
    <div class="main" :class="{'main-hide-text': hideMenuText}">
        <div class="sidebar-menu-con" :style="{width: hideMenuText?'60px':'200px', overflow: hideMenuText ? 'visible' : 'auto', background: $store.state.menuTheme === 'dark'?'#495060':'white'}">
            ...
            <sidebar-menu v-if="!hideMenuText" :menuList="menuList" :currentDepartment="currentDepartment" :iconSize="14"/>
            <sidebar-menu-shrink v-else :icon-color="menuIconColor" :currentDepartment="currentDepartment" :menuList="menuList"/>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            handleClickUserDropdown (name) {
                if (name === 'ownSpace') {
                    ...
                    // 退出登录
                    Cookies.remove('user');
                    Cookies.remove('pms-ss-id');
                    Cookies.remove('pms-X-UAId');
        ...
        created () {
            ...
            data.forEach(function(item, index) {
                if(item.give){
                    if(arr.indexOf(item.give) == -1) {
                    arr.push(item.give);
                    }
                }else{
                    console.error("请补全give属性");
                }
            });
            this.department = arr;
            this.currentDepartment = arr[0];
        }
    };
</script>
